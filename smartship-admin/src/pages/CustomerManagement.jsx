...
export default function CustomerManagement() {
  ...
  useEffect(() => {
    const fetchCustomers = async () => {
      try {
        // 1. Lấy khách hàng từ user-service
        const customerRes = await axios.get('http://localhost:8080/users/customers');
        // 2. Lấy Tên & SĐT từ auth-service
        const userRes = await axios.get('http://localhost:8080/auth/all-users-info');
        ...
        const realDataFromDB = customerRes.data;
        const mappedCustomers = realDataFromDB.map(user => {
          const rawId = user.userId;
          const userInfo = usersMap[rawId] || {}; // Ghép mảnh

          return {
            rawId: rawId, 
            id: `KH-${rawId}`, 
            name: userInfo.name || user.fullName || 'Khách hàng mới', 
            phone: userInfo.phone || user.phone || '090xxxxxxx',       
        ...
        console.error("Lỗi khi kéo danh sách khách hàng:", error);
      }
    };
    fetchCustomers();
  }, []);
  ...
  const handleToggleBlock = async (id, currentStatus, name) => {
    ...
    if(window.confirm(`Bạn có chắc muốn ${action} tài khoản của khách hàng ${name}?`)) {
      try {
        // Bồ nhớ bổ sung API call ở đây để lưu trạng thái vào DB nhé
        // await axios.patch(`http://localhost:8080/users/status/${id}`, { status: isBlocked ? 'ACTIVE' : 'BLOCKED' });
