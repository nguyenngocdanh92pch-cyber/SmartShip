...
export default function ShipmentManagement() {
  ...
  // Đưa hàm fetchShipments ra ngoài useEffect để gọi lại được ở bất cứ đâu
  ...
  const filteredOrders = shipments.filter(order => {
    const matchStatus = filterStatus === 'ALL' 
      ? true 
      : (filterStatus === 'COMPLETED_GROUP' 
          ? ['DELIVERED', 'COMPLETED', 'AT_WAREHOUSE'].includes(order.status) 
          : order.status === filterStatus);
          
    const keyword = String(searchTerm).toLowerCase().trim();
    
    // Thêm prefix ảo để gõ "SH", "KH", "TX" đều ra
    const matchSearch = 
        `sh-${order.id}`.toLowerCase().includes(keyword) || 
        (order.pickupAddress && order.pickupAddress.toLowerCase().includes(keyword)) ||
        (order.deliveryAddress && order.deliveryAddress.toLowerCase().includes(keyword)) ||
        (order.packageDescription && order.packageDescription.toLowerCase().includes(keyword)) ||
        (order.senderId && `kh #${order.senderId}`.toLowerCase().includes(keyword)) ||
        (order.senderId && String(order.senderId).includes(keyword)) ||
        (order.driverId && `tx #${order.driverId}`.toLowerCase().includes(keyword)) ||
  ...
  const getStatusBadge = (status) => {
    switch(status) {
      case 'WAITING_PAYMENT': return <span style={{...styles.badge, backgroundColor: '#E2E8F0', color: '#4A5568'}}>Chờ thanh toán</span>;
      // 🌟 ĐÃ THÊM: Case cho DA_THANH_TOAN
      case 'DA_THANH_TOAN': return <span style={{...styles.badge, backgroundColor: '#C6F6D5', color: '#22543D'}}>Đã thanh toán</span>;
      case 'PENDING': return <span style={{...styles.badge, backgroundColor: '#FEFCBF', color: '#B7791F'}}>Chờ tài xế</span>;
      case 'ACCEPTED': return <span style={{...styles.badge, backgroundColor: '#EBF8FF', color: '#3182CE'}}>Tài xế đã nhận</span>;
      case 'PICKED_UP': return <span style={{...styles.badge, backgroundColor: '#E9D8FD', color: '#805AD5'}}>Đã lấy hàng</span>;
      case 'AT_WAREHOUSE': return <span style={{...styles.badge, backgroundColor: '#FAF5FF', color: '#6B46C1'}}>Tại kho</span>;
      case 'DELIVERING': return <span style={{...styles.badge, backgroundColor: '#E6FFFA', color: '#234E52'}}>Đang giao hàng</span>;
      case 'DELIVERED':
    ...
    <div style={{ paddingBottom: '50px', position: 'relative' }}>
      ...
      <div style={{ display: 'flex', gap: '15px', marginBottom: showAdvancedFilter ? '15px' : '20px', flexWrap: 'wrap' }}>
        ...
        <select style={styles.selectInput} value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)}>
          ...
          {/* 🌟 ĐÃ THÊM: Option bộ lọc Đã thanh toán */}
          <option value="DA_THANH_TOAN">Đã thanh toán</option>
          <option value="PENDING">Chờ tài xế</option>
          <option value="ACCEPTED">Tài xế đã nhận</option>
          <option value="PICKED_UP">Đã lấy hàng</option>
          <option value="AT_WAREHOUSE">Tại kho</option>
          <option value="DELIVERING">Đang giao hàng</option>
          <option value="COMPLETED_GROUP">Hoàn thành</option>
            ...
            {loading ? (
              ...
              <tr key={order.id} style={styles.tr}>
                ...
                <td style={styles.td}>
                  ...
                  <div style={{ fontSize: '13px', marginBottom: '4px' }}><span style={{color:'#E53E3E', fontWeight:'bold'}}>Giao đến:</span> {order.deliveryAddress || 'Chưa cập nhật'}</div>
        ...
        <div style={styles.modalOverlay}>
          <div style={styles.modalContent}>
            ...
            <div style={{ display: 'flex', gap: '30px' }}>
              <div style={{ flex: 1 }}>
                <h4 style={{ margin: '0 0 15px 0', color: '#4A5568' }}>Thông tin hành trình</h4>
                <div style={{ backgroundColor: '#F7FAFC', padding: '15px', borderRadius: '8px', marginBottom: '20px' }}>
                  ...
                  <div style={{ display: 'flex', gap: '10px', marginBottom: '10px' }}><FaMapMarkerAlt color="#E53E3E"/> <div><small>Địa chỉ giao đến:</small><br/><strong>{selectedOrder.deliveryAddress || 'Chưa cập nhật'}</strong></div></div>
              ...
              <div style={{ flex: 1 }}>
                ...
                {/* ========================================================= */}
                {/* 🎯 KHỐI LỊCH SỬ THANH TOÁN (FIX LOGIC THANH TOÁN XONG -> CHỜ TÀI XẾ) */}
                {/* ========================================================= */}
                <div style={{ marginTop: '15px', padding: '15px', backgroundColor: '#F7FAFC', borderRadius: '8px' }}>
                  <h4 style={{ margin: '0 0 10px 0', color: '#2D3748' }}>Chi tiết thanh toán</h4>
                  <p style={{ margin: '5px 0', fontSize: '16px' }}>
                    <strong>Trạng thái: </strong> 
                    
                    {/* Kiểm tra nếu trạng thái đã vượt qua bước chờ thanh toán */}
                    {['DA_THANH_TOAN', 'PENDING', 'ACCEPTED', 'PICKED_UP', 'AT_WAREHOUSE', 'DELIVERING', 'DELIVERED', 'COMPLETED'].includes(selectedOrder.status) ? (
                      <span style={{ color: 'green', fontWeight: 'bold' }}>Thành công</span>
                    ) : (
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px', marginTop: '25px', paddingTop: '15px', borderTop: '1px solid #E2E8F0' }}>
              {selectedOrder.status !== 'DELIVERED' && selectedOrder.status !== 'COMPLETED' && selectedOrder.status !== 'CANCELLED' && selectedOrder.status !== 'AT_WAREHOUSE' && (
