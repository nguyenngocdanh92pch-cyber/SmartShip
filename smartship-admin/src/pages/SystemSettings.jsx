...
import { FaSave, FaCogs, FaMapMarkerAlt, FaPlus, FaShieldAlt, FaArrowRight, FaMoneyBillWave, FaTrophy, FaTrash } from 'react-icons/fa';
...
export default function SystemSettings() {
    ...
    axios.get('http://localhost:8080/shipments/settings')
      .then(res => {
        ...
        if (Object.keys(data).length > 0) {
          setSettings(prev => {
            // Lôi chuỗi JSON của Geofencing từ Backend ra và dịch lại thành Mảng
            let loadedGeofencing = prev.geofencing;
            if (data['geofencing_zones']) {
              try {
                loadedGeofencing = JSON.parse(data['geofencing_zones']);
              } catch (e) {
                console.error("Lỗi parse JSON geofencing", e);
              }
            }
            ...
            return {
              ...
              geofencing: loadedGeofencing // Cập nhật dữ liệu thật vào State
            };
          });
        }
      })
  ...
  // Hàm thêm khu vực Geofencing mới
  const handleAddGeofence = () => {
    setSettings(prev => ({
      ...prev,
      geofencing: [...prev.geofencing, { id: Date.now(), name: '', fee: 0 }]
    }));
  };

  // Hàm cập nhật khu vực Geofencing khi gõ chữ/số
  const handleGeofenceChange = (id, field, value) => {
    setSettings(prev => ({
      ...prev,
      geofencing: prev.geofencing.map(zone => 
        zone.id === id ? { ...zone, [field]: field === 'fee' ? parseInt(value) || 0 : value } : zone
      )
    }));
  };

  // Hàm xóa khu vực Geofencing
  const handleDeleteGeofence = (id) => {
    setSettings(prev => ({
      ...prev,
      geofencing: prev.geofencing.filter(zone => zone.id !== id)
    }));
  };

  // Lấy tọa độ từ tên địa điểm (Dùng API Mapbox)
  const getCoordinatesFromAddress = async (address) => {
    try {
      const MAPBOX_TOKEN = 'pk.eyJ1IjoibmdvY2RhbmgwMjAzMjAwNSIsImEiOiJjbW85ZmluejMwOGs4MndvaXU1MDc4aG1xIn0.R9tJL97Mm909Xwt1GE4I3w';
      const response = await axios.get(
        `https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(address)}.json`,
        { params: { access_token: MAPBOX_TOKEN, country: 'vn', limit: 1 } }
      );
      ...
      console.error("Lỗi gọi Mapbox API:", error);
      return null;
    }
  };

  // Lưu cấu hình xuống Backend (ĐÃ NÂNG CẤP)
  const handleSaveOperation = async () => {
    try {
      // 1. Quét qua các khu vực phụ phí để lấy tọa độ Mapbox
      const geofencingWithCoords = await Promise.all(settings.geofencing.map(async (zone) => {
        const coords = await getCoordinatesFromAddress(zone.name);
        if(coords) {
            console.log(`🌍 Đã tìm thấy tọa độ ${zone.name}: Lat ${coords.latitude}, Lng ${coords.longitude}`);
        }
        return {
            ...zone,
      ...
      const payload = {
        ...
        // Gửi mảng khu vực phụ phí kèm tọa độ
        'geofencing_zones': JSON.stringify(geofencingWithCoords) 
      };
        ...
        <div style={styles.fadeAnimation}>
          ...
          <div style={styles.card}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '2px solid #EBF8FF', paddingBottom: '10px', marginBottom: '20px' }}>
              ...
              <button onClick={handleAddGeofence} style={styles.addBtn}><FaPlus /> Thêm khu vực</button> 
            </div>
            {settings.geofencing.map(zone => (
              <div key={zone.id} style={{ display: 'flex', gap: '20px', marginBottom: '15px', alignItems: 'center' }}>
                ...
                <input 
                  type="text" 
                  placeholder="Nhập tên địa điểm (VD: Sân bay Tân Sơn Nhất)"
                  style={{...styles.input, flex: 2}} 
                  value={zone.name} 
                  onChange={(e) => handleGeofenceChange(zone.id, 'name', e.target.value)} 
                />
                <div style={styles.inputGroup}>
                  <input 
                    ...
                    style={styles.input} 
                    value={zone.fee} 
                    onChange={(e) => handleGeofenceChange(zone.id, 'fee', e.target.value)} 
                  /> 
                  <span style={styles.unit}>VNĐ</span>
                </div>
...
const styles = {
  ...
  deleteBtn: { backgroundColor: '#FED7D7', color: '#E53E3E', border: 'none', padding: '10px 15px', borderRadius: '6px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' },
};