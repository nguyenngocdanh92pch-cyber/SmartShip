...
class ApiService {
  ...
  // 1. Hàm lấy số lượng chấm đỏ
  static Future<int> getUnreadNotificationCount(int userId) async {
    final response = await http.get(Uri.parse('${ApiConfig.baseUrl}/api/notifications/unread-count/$userId'));
    if (response.statusCode == 200) {
  ...
  static Future<List<dynamic>> getMyNotifications(int userId) async {
    final response = await http.get(Uri.parse('${ApiConfig.baseUrl}/api/notifications/me/$userId'));
    if (response.statusCode == 200) {
  ...
  static Future<void> markNotificationAsRead(int logId) async {
    await http.put(Uri.parse('${ApiConfig.baseUrl}/api/notifications/$logId/read'));
  }
}
