...
public class AuthService {
    ...
    // Gộp cả RestTemplate (gọi API) và RabbitMQ (bắn thông điệp)
    ...
    public AuthResponse login(LoginRequest request) {
        ...
        // ===============================================================
        // 🛡️ 3. NGƯỜI BẢO VỆ: KIỂM TRA TRẠNG THÁI BÊN USER-SERVICE
        // (Logic chặn tài khoản chưa được duyệt của bồ)
        // ===============================================================
        if (User.Role.DRIVER.equals(user.getRole())) {
            try {
                // 🎯 Đã ghim cứng Port 8082 của user-service vào đây
                String userServiceUrl = "http://localhost:8082/users/status/" + user.getId();

                ResponseEntity<String> response = restTemplate.getForEntity(userServiceUrl, String.class);
                String status = response.getBody();

                if ("PENDING".equals(status)) {
                    throw new RuntimeException("Tài khoản của bạn đang chờ Admin kiểm duyệt. Vui lòng quay lại sau!");
                } else if ("LOCKED".equals(status)) {
                    throw new RuntimeException("Tài khoản đã bị khóa do vi phạm!");
                }
            } catch (RuntimeException re) {
                throw re; // Bắn lỗi ra để Controller bắt và trả về cho App
            } catch (Exception e) {
                System.err.println("Lỗi gọi sang User-Service: " + e.getMessage());
                throw new RuntimeException("Hệ thống đang bảo trì hoặc lỗi mạng. Vui lòng thử lại sau!");
            }
        }

        // 4. Sinh JWT Token có chứa thông tin Role
        String token = jwtUtil.generateToken(user.getPhoneNumber(), user.getRole().name());

        // 5. Trả về DTO chứa token và role cho phía Client phân luồng
    ...
    // =========================================================
    // LOGIC MỚI TỪ BẠN BỒ: CẬP NHẬT VÀ ĐỒNG BỘ QUA RABBITMQ
    // =========================================================
    ...
    // =========================================================
    // LOGIC MỚI TỪ BẠN BỒ: ĐỔI MẬT KHẨU
    // =========================================================
