...
import 'address_search_screen.dart'; // 🌟 Import màn hình tìm kiếm địa chỉ vào đây
...
class _CreateOrderScreenState extends State<CreateOrderScreen> {
  // 🌟 KHAI BÁO CÁC BIẾN LƯU TRỮ ĐỊA CHỈ & TỌA ĐỘ
  ...
  // Controllers quản lý dữ liệu hàng hóa
  ...
  File? _selectedImage; // Biến lưu trữ ảnh được chọn
  bool _isLoading = false;

  final String apiGatewayUrl = ApiConfig.baseUrl; // Đảm bảo đúng IP Backend
  ...
  Future<void> _selectAddress(bool isPickup) async {
    ...
    // Nhận dữ liệu trả về và cập nhật UI
    if (result != null) {
      setState(() {
        if (isPickup) {
  ...
  Future<void> _handleCreateOrder() async {
    // 1. Kiểm tra tính hợp lệ (Sử dụng biến thay vì Controller)
    if (pickupAddress == null ||
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            "Vui lòng chọn địa chỉ và nhập đầy đủ thông tin hàng hóa!",
          ),
        ),
      );
      return;
    ...
    try {
      ...
      // 3. Chuẩn bị Request Multipart (Form-Data)
      ...
      // 🌟 TRUYỀN TRỰC TIẾP TỌA ĐỘ ĐÃ LƯU XUỐNG BACKEND
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            // 🌟 KHỐI ĐỊA CHỈ MỚI (TÍCH HỢP TÌM KIẾM MAPBOX)
            ...
            _buildInputCard([
              _buildTextField(
                "Mô tả hàng hóa (ví dụ: Quần áo, Đồ điện tử)",
                Icons.inventory_2,
                Colors.orange,
              ...
              _buildTextField(
                ...
                keyboardType: TextInputType.number, // Bàn phím số cho giá trị
              ),
              const Divider(height: 20),
  ...
  // 🌟 WIDGET MỚI: NÚT BẤM CHỌN ĐỊA CHỈ
