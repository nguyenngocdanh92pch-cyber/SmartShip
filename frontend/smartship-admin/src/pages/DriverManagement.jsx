...
export default function DriverManagement() {
  ...
  const [selectedDriver, setSelectedDriver] = useState(null); // Cho nạp ví
  const [topupAmount, setTopupAmount] = useState('');
  
  const [reviewProfile, setReviewProfile] = useState(null); // Cho duyệt hồ sơ
  
  // State quản lý ảnh đang được phóng to
  const [zoomedImage, setZoomedImage] = useState(null); 

  // State quản lý danh sách tài xế
  const [drivers, setDrivers] = useState([]);

  // KÉO DỮ LIỆU THẬT TỪ BACKEND KHI TRANG LOAD
  useEffect(() => {
    const fetchDrivers = async () => {
      try {
        ...
        const mappedDrivers = realData.map(d => ({
          ...
          // 🚀 1. ĐÃ THÊM HỨNG DỮ LIỆU ĐỊA CHỈ TẠI ĐÂY
          ...
          profile: {
            ...
            vehicle_info: { brand: 'Đang cập nhật', model: '...', plate: '...', color: '...' } 
          }
        }));
  ...
  const handleApproveAction = async (id, rawId, actionType) => {
    if (actionType === 'REJECT') {
      ...
      if(reason) {
        ...
        try {
            await axios.put('http://localhost:8080/users/me', {
                userId: rawId,
                status: 'ACTIVE'
            });
            setDrivers(drivers.map(d => d.id === id ? { ...d, status: 'ONLINE' } : d));
            alert(`Đã phê duyệt hồ sơ thành công! Tài khoản được kích hoạt.`);
        } catch (error) {
  ...
  return (
    <div style={{ paddingBottom: '50px' }}>
              ...
              <tr key={d.id} style={styles.tr}>
                ...
                <td style={styles.td}>
                  {d.status === 'PENDING_APPROVAL' ? (
                    <button onClick={() => setReviewProfile(d)} style={styles.approveBtn}><FaIdCard /> Kiểm tra hồ sơ</button>
                  ) : (
                    <div style={{ display: 'flex', gap: '10px' }}>
                      <button onClick={() => setSelectedDriver(d)} style={styles.actionBtn}><FaMoneyBillWave color="#38A169" /> Nạp ví</button>
                      {d.status === 'LOCKED' || d.status === 'BLOCKED' ? (
                        <button onClick={() => handleToggleLock(d.id, d.rawId, d.status, d.name)} style={styles.unlockBtn}><FaUnlock /> Mở khóa</button>
                      ) : (
                        <button onClick={() => handleToggleLock(d.id, d.rawId, d.status, d.name)} style={styles.dangerBtn}><FaLock /> Khóa</button>
                      )}
                    </div>
                  )}
                </td>
      ...
      {/* MODAL KIỂM TRA HỒ SƠ TÀI XẾ */}
      {reviewProfile && (
        <div style={styles.modalOverlay}>
          <div style={{...styles.modalContent, width: '900px'}}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #E2E8F0', paddingBottom: '15px', marginBottom: '20px' }}>
              <h2 style={{ margin: 0, color: '#2D3748', display: 'flex', alignItems: 'center', gap: '10px' }}><FaIdCard color="#1E88E5"/> Duyệt hồ sơ Đối tác</h2>
            ...
            <div style={{ display: 'flex', gap: '25px', marginBottom: '20px' }}>
              ...
              <div style={{ flex: 1.5 }}>
                ...
                <div style={styles.docsGrid}>
                  ...
                  {/* Ảnh CCCD Mặt Sau */}
                  <div style={styles.imageBox}>
                    <p style={styles.imageLabel}>CCCD/CMND (Mặt sau)</p>
                    <img 
                      src={reviewProfile.profile?.id_card_back_url} 
                      style={styles.docImage} 
                      onClick={() => setZoomedImage(reviewProfile.profile?.id_card_back_url)}
                      alt="CCCD Sau" 
                    />
                  </div>
                  ...
                  {/* Ảnh Cà vẹt xe */}
                  <div style={styles.imageBox}>
                    <p style={styles.imageLabel}>Giấy đăng ký xe (Cà vẹt)</p>
                    <img 
                      src={reviewProfile.profile?.vehicle_reg_url} 
                      style={styles.docImage} 
                      onClick={() => setZoomedImage(reviewProfile.profile?.vehicle_reg_url)}
                      alt="Đăng ký xe" 
                    />
                  </div>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ backgroundColor: '#F7FAFC', padding: '15px', borderRadius: '8px', marginBottom: '15px', border: '1px solid #E2E8F0' }}>
                  ...
                  {/* 🚀 2. ĐÃ CẬP NHẬT GIAO DIỆN CHỖ NÀY */}
...
const styles = {
  ...
  modalContent: { backgroundColor: 'white', padding: '30px', borderRadius: '15px', maxWidth: '90%', boxShadow: '0 10px 25px rgba(0,0,0,0.2)' },
