...
// 🎯 IMPORT CỤC LEGO CÁI CHUÔNG THÔNG BÁO VÀO ĐÂY
...
class OrderTrackingModel {
  ...
  factory OrderTrackingModel.fromJson(Map<String, dynamic> json) {
    ...
    return OrderTrackingModel(
      ...
      driverId: parseIntSafe(json['driverId']) ?? parseIntSafe(json['driver_id']),
      driverName: json['driverName']?.toString() ?? json['driver_name']?.toString() ?? 'Tài xế',
      deliveryAddress: json['deliveryAddress']?.toString() ?? 'Đang cập nhật địa chỉ...',
      ...
      packageDescription: json['packageDescription']?.toString() ?? 'Không có mô tả',
...
/// ---------------------------------------------------------------------------
/// SCREEN: GIAO DIỆN THEO DÕI ĐƠN HÀNG
/// ---------------------------------------------------------------------------
...
class _TrackingScreenState extends State<TrackingScreen> {
  ...
  Widget build(BuildContext context) {
    if (widget.orderId != null) {
      return Scaffold(
        ...
        appBar: AppBar(
          ...
          elevation: 0,
          // 🎯 GẮN CHUÔNG VÀO GÓC PHẢI TRANG CHI TIẾT
          actions: const [
            NotificationBell(),
          ],
        ),
    ...
    final ongoingOrders = _allOrders.where((o) => !['DELIVERED', 'COMPLETED', 'CANCELLED'].contains(o.status.toUpperCase())).toList();
    final historyOrders = _allOrders.where((o) => ['DELIVERED', 'COMPLETED', 'CANCELLED'].contains(o.status.toUpperCase())).toList();
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          // 🎯 GẮN CHUÔNG VÀO GÓC PHẢI TRANG DANH SÁCH
          actions: const [
            NotificationBell(),
          ],
          bottom: const TabBar(
  ...
  Widget _buildOrderCard(OrderTrackingModel order) {
    bool isCompleted = order.status.toUpperCase() == 'DELIVERED' || order.status.toUpperCase() == 'COMPLETED';
    bool canRate = isCompleted && order.rating == null;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                Text(
                  ['PENDING', 'DA_THANH_TOAN', 'ACCEPTED', 'PICKED_UP', 'AT_WAREHOUSE', 'DELIVERING', 'IN_TRANSIT', 'DELIVERED', 'COMPLETED'].contains(order.status.toUpperCase()) 
                      ? "Thành công" 
                      : "Chưa thanh toán",
                  style: TextStyle(
                    color: ['PENDING', 'DA_THANH_TOAN', 'ACCEPTED', 'PICKED_UP', 'AT_WAREHOUSE', 'DELIVERING', 'IN_TRANSIT', 'DELIVERED', 'COMPLETED'].contains(order.status.toUpperCase()) 
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    if (['DELIVERING', 'IN_TRANSIT', 'PICKED_UP'].contains(order.status.toUpperCase())) ...[
          ...
          if (isCompleted || order.status.toUpperCase() == 'AT_WAREHOUSE') ...[
            const Padding(
              padding: EdgeInsets.symmetric(vertical: 12),
              child: Divider(height: 1, color: Color(0xFFEEEEEE)),
            ),
